...
for bin in bash curl docker jq python3 sha256sum tar; do
...
for _ in $(seq 1 180); do
  download_url="$(sed -n 's|.*\(http://[^[:space:]]*#sha256=[[:xdigit:]]\{64\}\).*|\1|p' \
    "$backup_log" | tail -n1)"
...
RESTORE_BASE="$restore_base" bash "$ROOT_DIR/docker_migrate_perfect.sh" \
...
echo "End-to-end backup, HTTP transfer, and restore test passed"
